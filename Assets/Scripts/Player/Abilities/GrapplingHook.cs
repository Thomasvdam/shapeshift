...
namespace Assets.Scripts
{
	public class GrapplingHook : MonoBehaviour
	{
		...
		void Start ()
		{
			...
			_grapple.GetComponent<Rigidbody2D> ().isKinematic = false;
		...
		private void updatePull ()
		{
			...
			grappledObj.GetComponent<Rigidbody2D> ().AddForce (new Vector2 (pullVector.x * 20f, -pullVector.y * 35f));
		}
		}
			
		private void CheckForGrapple ()
		{
			if (controller.getKeyDown (Controller.LEFT_BUMPER_PS4)) {
				// Get joystick direction. TODO get this from other object/script
				float x = controller.getXAxisRaw ();
				float y = controller.getYAxisRaw ();
...
//            else*/
			if (!controller.getKey (Controller.LEFT_BUMPER_PS4)) {
				RetractRope ();
			} else if (controller.getKeyDown (Controller.RIGHT_BUMPER_PS4)) {
				if (_points.Count <= 1) {
					Debug.Log ("first here: " + grappledObj.name);
...
//					grappledObj.GetComponent<Rigidbody2D>().AddForce (pullDirection.normalized * pullForce);
					...
					//_joint.connectedBody.AddForce (new Vector2 (transform.right.x * 20f, 0));
					pullVector = new Vector2(controller.getXAxisRaw(), controller.getYAxisRaw());
					grappledObj.GetComponent<Rigidbody2D> ().AddForce (new Vector2 (pullVector.x * 20f, -pullVector.y * 35f));
				}
				...
				GetComponent<Rigidbody2D> ().AddForce (Vector3.right * controller.getXAxisRaw () * 25);
				_joint.distance += controller.getYAxisRaw () * Time.deltaTime * 2;
